...
def masked_mean_otr_balanced(values, mask, prompt_indices, axis=None):
    """
    OTR(Optimal Truncation Resampling) 샘플링 모드에서 사용하는 균형잡힌 평균 계산: 프롬프트별 평균 → 배치 평균
    
    Args:
        values: (batch_size, seq_len) - 손실 값들
        mask: (batch_size, seq_len) - 마스크
        prompt_indices: (batch_size,) - 각 시퀀스가 속한 프롬프트 ID
        axis: 평균을 구할 축
    
    Returns:
        균형잡힌 평균값 (프롬프트별 평균의 배치 평균)
    """
    if prompt_indices is None:
        # OTR 샘플링이 비활성화된 경우 기존 방식 사용
        return masked_mean(values, mask, axis)
    
    # 시퀀스별 평균 계산 (axis=-1로 시퀀스 차원 평균)
    sequence_means = masked_mean(values, mask, axis=-1)  # (batch_size,)
    ...
    for prompt_id in unique_prompts:
        prompt_mask = (prompt_indices == prompt_id)
        if prompt_mask.sum() > 0:
            prompt_mean = sequence_means[prompt_mask].mean()
            prompt_means.append(prompt_mean)
    
    if len(prompt_means) > 0:
        # 프롬프트별 평균들의 배치 평균
        return torch.stack(prompt_means).mean()
    else:
        # fallback
        return masked_mean(values, mask, axis)
