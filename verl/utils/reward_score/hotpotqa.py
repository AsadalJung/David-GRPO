...
def extract_search_result_blocks(solution_str):
    pattern = r'<search_result>(.*?)</search_result>'
    matches = re.findall(pattern, solution_str, re.DOTALL)
    return [match.strip() for match in matches]


def compute_answer_in_search_reward(solution_str, ground_truth, partial_reward_score=0.5):
    if not solution_str or not ground_truth:
        return 0.0

    answers = ground_truth if isinstance(ground_truth, (list, tuple)) else [ground_truth]
    search_blocks = extract_search_result_blocks(solution_str)
    if not search_blocks:
        return 0.0

    normalized_blocks = [normalize_answer(block) for block in search_blocks]
    for answer in answers:
        normalized_answer = normalize_answer(answer)
        if not normalized_answer:
            continue
        for block in normalized_blocks:
            if normalized_answer in block:
                return partial_reward_score

    return 0.0


def _normalize_partial_reward_weight(value, default=0.5):
    try:
        weight = float(value)
...
def default_compute_score(solution_str, ground_truth, method='strict', format_score=0.50, score=1.0, extra_info=None):
   ...
   # <think> 이후에서 검색시도 안했을 경우 0점 
    ...
    if extra_info and 'supporting_facts' in extra_info:
        ...
        partial_reward_weight = _normalize_partial_reward_weight(
            trainer_config.get('partial_reward_weight', 0.5)
        )
        ...
        if give_partial_reward:
            ...
            if use_answer_in_search_reward:
                partial_reward = compute_answer_in_search_reward(
                    response_part,
                    ground_truth,
                    partial_reward_score=partial_reward_weight,
                )
            else:
                supporting_facts = extra_info.get('supporting_facts', {})
                partial_reward = compute_partial_reward(
                    response_part,
                    supporting_facts,
                    partial_reward_score=partial_reward_weight,
                )
