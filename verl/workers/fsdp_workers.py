...
def _override_optimizer_lr(optimizer, lr_scheduler, new_lr: float):
    """
    Scale current lr by the same schedule factor but replace the base lr with `new_lr`.
    This keeps step/optimizer state while letting us change lr mid-run.
    """
    ...
    for group in optimizer.param_groups:
        ...
        scale = group['lr'] / base_lr if base_lr else 1.0
    ...
    base_lrs = lr_scheduler.base_lrs
    last_lr = getattr(lr_scheduler, '_last_lr', None)
    if last_lr is not None and len(last_lr) == len(base_lrs):
        multipliers = [lr / base if base else 1.0 for lr, base in zip(last_lr, base_lrs)]
    else:
        multipliers = [group['lr'] / new_lr if new_lr else 1.0 for group in optimizer.param_groups]

    lr_scheduler.base_lrs = [new_lr for _ in base_lrs]
    lr_scheduler._last_lr = [new_lr * m for m in multipliers]
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        if role == 'actor':
            ...
            min_lr_ratio = optim_config.get('min_lr_ratio', 0.) or 0.

            print(f'Total steps: {total_steps}, num_warmup_steps: {num_warmup_steps}, warmup_style: {warmup_style}')
            ...
            if warmup_style == 'cosine':
                ...
                raise ValueError(f'Unsupported warmup_style: {warmup_style}')
        else:
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
            ...
            from verl.workers.rollout.vllm_rollout import vLLMRollout, vllm_mode, vLLMRollout_Search
            from verl.workers.sharding_manager import FSDPVLLMShardingManager
            log_gpu_memory_usage('Before building vllm rollout', logger=None)
            local_path = copy_local_path_from_hdfs(self.config.model.path)
            if vllm_mode == 'customized':
                if self.config.rollout.model == 'search':
                    rollout = vLLMRollout_Search(actor_module=self.actor_module_fsdp,
                ...
                if self.config.rollout.model == 'search':
                    rollout = vLLMRollout_Search(model_path=local_path,
                                                 config=self.config.rollout,
                                                 tokenizer=self.tokenizer,
                                                 model_hf_config=self.actor_model_config,
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def set_rollout_max_search(self, max_search_nums: int):
        """vLLM rollout의 최대 검색 횟수를 동적으로 갱신한다."""
        if not self._is_rollout:
            return
        if not hasattr(self, 'rollout'):
            return
        if hasattr(self.rollout, 'set_max_search_nums'):
            self.rollout.set_max_search_nums(max_search_nums)
        else:
            try:
                self.rollout.max_search_nums = int(max_search_nums)
            except Exception:
                return
    ...
    def override_lr(self, lr: float):
        ...
        _override_optimizer_lr(self.actor_optimizer, self.actor_lr_scheduler, lr)
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        min_lr_ratio = config.optim.get('min_lr_ratio', 0.) or 0.
    ...
    def override_lr(self, lr: float):
        ...
        _override_optimizer_lr(self.critic_optimizer, self.critic_lr_scheduler, lr)
