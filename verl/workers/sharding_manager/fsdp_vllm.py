...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def preprocess_data(self, data: DataProto) -> DataProto:
        ...
        # 🔍 Sharding Manager에서 데이터 처리 디버깅
        tp_size = vllm_ps.get_tensor_model_parallel_world_size()
        print(f"\n🔧 FSDPVLLMShardingManager.preprocess_data:")
        print(f"   📊 Before allgather - batch size: {len(data.batch) if data.batch is not None else 0}")
        print(f"   📊 Before allgather - non_tensor keys: {list(data.non_tensor_batch.keys())}")
        if 'supporting_facts' in data.non_tensor_batch:
            sf = data.non_tensor_batch['supporting_facts']
            print(f"   📊 Before allgather - supporting_facts size: {len(sf)}")
        ...
        # 🔧 non_tensor_batch도 allgather 처리 (supporting_facts 포함)
        if data.non_tensor_batch and tp_size > 1:
            import torch
            import numpy as np
            
            # 모든 워커에서 non_tensor_batch 수집
            ...
            # non_tensor_batch의 각 키에 대해 concatenate
            for key in data.non_tensor_batch.keys():
                # 모든 워커의 데이터를 하나로 합치기
                all_data_for_key = [batch_data[key] for batch_data in all_non_tensor_batch]
                data.non_tensor_batch[key] = np.concatenate(all_data_for_key, axis=0)
        
        print(f"   ✅ After allgather - batch size: {len(data.batch) if data.batch is not None else 0}")
        if 'supporting_facts' in data.non_tensor_batch:
            sf = data.non_tensor_batch['supporting_facts']
            print(f"   ✅ After allgather - supporting_facts size: {len(sf)}")
        print(f"🔧 FSDPVLLMShardingManager.preprocess_data completed\n")
